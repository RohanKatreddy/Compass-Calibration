...
def enforce_hard_iron(max_magnitude, min_magnitude, max_magnitude_points, min_magnitude_points, x, y, z, x_offset, y_offset, z_offset):
    ...
    for i in range(len(x)):
        ...
        y[i] = y[i] + -(y_offset)
        z[i] = z[i] + -(z_offset)
    return max_magnitude, min_magnitude, max_magnitude_points, min_magnitude_points, x, y, z
...
data_file = open("C:\\Rohan\\Coding\\Python\\Projects\\AutonomousCarProject\\Magnetometer\\MagnetometerV2\\raw_mag_data copy.txt", 'r')

# Initialize variables
max_magnitude, min_magnitude, max_z, min_z = 0, 999, 0, 999
max_magnitude_points, min_magnitude_points, max_z_p, min_z_p = [0,0], [0,0], [0,0], [0,0]
rotated_x_values, rotated_y_values = [], []
...
#get magnitudes for z and x
for i in range(len(z)):
    m = math.sqrt(float(x[i])**2 + float(z[i])**2)
    if m > max_z:
        max_z = m
        max_z_p = [z[i], x[i]]
...
max_magnitude, min_magnitude, max_magnitude_points, min_magnitude_points, x, y, z = enforce_hard_iron(max_magnitude, min_magnitude, max_magnitude_points, min_magnitude_points, x, y, z, x_offset, y_offset, z_offset)
...
theta_z = math.asin((max_z_p[0] / max_z))
cos_theta_z = math.cos(theta_z)
sin_theta_z = math.sin(theta_z)
r_z = [(cos_theta_z, -sin_theta_z), ((sin_theta_z), cos_theta_z)]
scale_factor_x = min_z/max_z
rotated_z, rotated_x = allign_major_axis_with_x_axis(r_z, z, x, scale_factor_x)
cos_theta_z = math.cos(-theta_z)
sin_theta_z = math.sin(-theta_z)
r_z = [(cos_theta_z, -sin_theta_z), ((sin_theta_z), cos_theta_z)]
calibrated_z, calibrated_x = allign_major_axis_with_x_axis(r_z, z, x, 1)
